...
fn ray_color(r:Ray,sphere:Sphere)->[u8;3]{
    ...
    if t > 0.0{
        ...
        color((1.0-t)*1.0+0.5*t,(1.0-t)*1.0+0.7*t,(1.0-t)*1.0+1.0*t)
    }
}

pub fn render(data:Data,bar: ProgressBar) ->ImageBuffer<Rgb<u8>, Vec<u8>>{
    let mut img: RgbImage = ImageBuffer::new(data.width.try_into().unwrap(), data.height.try_into().unwrap());
    ...
    for j in 0..data.height{
        for i in 0..data.width{
            ...
            let dir = data.lower_left_corner.clone() + data.horizontal.clone() * u + data.vertical.clone() * v;
